...
### Uses the function create_opus_pair.py 
import os
import sys
...
for i in range(len(langs)-1):
	for j in range(i+1,len(langs)):
		...
		# Appending the path of the input base to the OPUS path
		...
		######## Commenting a command below	
		### Calling the pair creation function for all the sentences
		cmd = 'python3 create_opus_pair.py ' + langs[i] + ' ' + langs[j] + ' ' + base + ' overall.txt'
		os.system(cmd)
		
		# Coming back to the original Directory where the files are present
		os.chdir(cwd)

		### Calling the pair creation function for all the sentences above the threshold 
		cmd = 'python3 create_opus_pair.py ' + langs[i] + ' ' + langs[j] + ' ' + base + ' t_' + thr + '.txt'  
		os.system(cmd)
